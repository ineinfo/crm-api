...
router.put('/updatestatus',upload.fields([{ name: 'document' }]), authenticateToken, async (req, res) => {
    ...
    try {
        ...
        if(lead_status == 8) {
            ...
            if(!inv_sale_date || !inv_seller_developer_name || !inv_property_address || !inv_apartment_type || !inv_buyer_name || !inv_selling_real_estate_company_name || !inv_selling_agent_name || !inv_total_purchase_price || !inv_commission || !inv_selling_brokerage_commission || !inv_selling_brokerage_currency || !inv_closing_note || !inv_prepared_by || !inv_name || !inv_company_name || !inv_phone || !inv_email || !inv_address || !inv_bank_name || !inv_bank_account_name || !inv_bank_account_number || !inv_bank_sort_code || !inv_bank_reference) {
                return res.status(400).json({ message: 'Please provide all invoice information', status: 'error' });
            }
            let inv_db_sale_date = formatDateForDB(inv_sale_date);
            [result] = await pool.query(`INSERT INTO ${TABLE.LEAD_SALES_STATUS_LIST_TABLE} (lead_id, user_id, lead_status, inv_sale_date, inv_seller_developer_name, inv_property_address, inv_apartment_type, inv_buyer_name, inv_selling_real_estate_company_name, inv_selling_agent_name, inv_total_purchase_price, inv_commission, inv_selling_brokerage_commission, inv_selling_brokerage_currency, inv_closing_note, inv_prepared_by, inv_name, inv_company_name, inv_phone, inv_email, inv_address, inv_bank_name, inv_bank_account_name, inv_bank_account_number, inv_bank_sort_code, inv_bank_reference) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [lead_id, user_id, lead_status, inv_db_sale_date, inv_seller_developer_name, inv_property_address, inv_apartment_type, inv_buyer_name, inv_selling_real_estate_company_name, inv_selling_agent_name, inv_total_purchase_price, inv_commission, inv_selling_brokerage_commission, inv_selling_brokerage_currency, inv_closing_note, inv_prepared_by, inv_name, inv_company_name, inv_phone, inv_email, inv_address, inv_bank_name, inv_bank_account_name, inv_bank_account_number, inv_bank_sort_code, inv_bank_reference]);
        }
        ...
        if(lead_status == 11) {
            ...
            const completeDocumentUrl = req.files['document'] 
            ? `${req.protocol}://${req.get('host')}/lead_complete_documents/${req.files['document'][0].filename}` 
            : null;
            
            [result] = await pool.query(`INSERT INTO ${TABLE.LEAD_SALES_STATUS_LIST_TABLE} (lead_id, user_id, lead_status, completion_date, complete_document_url) VALUES (?, ?, ?, ?, ?)`, [lead_id, user_id, lead_status, completion_date_db, completeDocumentUrl]);
